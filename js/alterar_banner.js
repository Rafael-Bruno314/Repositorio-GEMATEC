...
function loadDoc(cFunction) {
  ...
  var url = "php/select_banner.php?manda="+encodeURIComponent(selecionado);
...
function Tratar_resposta(resposta){ //
	...
	document.getElementById("autor").value = res[0];
		
	document.getElementById("titulo").value = res[1];

	document.getElementById("palavras_chave").value = res[2];
	
	document.getElementById("ano").value = res[3];
