...
function Manda_categoria(categoria){
	if(categoria){
		document.getElementById("trapaca_tipo").value = categoria;
		var cat = document.getElementById("tipo");
...
function loadDoc(cFunction) {
  ...
  var url = "php/select_artigo.php?manda="+encodeURIComponent(selecionado);
...
function Tratar_resposta(resposta){ //
	...
	document.getElementById("palavras_chave").value = res[2];
	
	document.getElementById("tipo").value = res[3];
