...
if (isset($_POST['cad_dps_da_ganbiarra'])) {
    ...
    /*if ($titulo == "" || $autor == "" || $palavras_chave == "" || $ano == "Ano de Publicação" || empty($_FILES['banner']['name'])) {
        echo "<script>var ola = confirm('Nem todos os campos foram preenchidos\\ndeseja realmete cadastrar?')</script>";
		echo "<script> if(ola){ alert('Tudo bem');} </script>";
	} else { */
		...
		if(!empty($_FILES['banner']['name'])){
			...
			#$sql = "INSERT INTO usuarios(autor,titulo,palavras_chave,ano) VALUES ('$autor','$titulo','$palavras_chave','$ano')"; //Linha problema!!!
		}
	...
	<div class="container">
		...
		<form class="form-horizontal" action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post" enctype="multipart/form-data" name="cadastro" >
			...
			<table width = "100%" border = "0" frame="none">
				...
				<div class="col-sm-10"><td>
					<input type="file" name="banner" class="form-control" id="banner"></td></tr>
				 </div>
			</table>
			<hr>
